  ...
  if (metadata->their_size != ply->size){
	...
	case MESSAGE_BLOCK_TYPE_ACK:
	  if (!found_their_ack){
	    ...
	    DEBUGF(meshms, "Found their last ack @%"PRIu64, metadata->their_last_ack_offset);
	  }
	  break;
      }
    }
...
enum meshms_status meshms_message_iterator_open(struct meshms_message_iterator *iter, const sid_t *me, const sid_t *them)
{
  ...
  while(c){
    if (cmp_sid_t(them, &c->them)==0){
      DEBUGF(meshms, "Found matching conversation, known=%d, found_mine=%d, found_theirs=%d, read_offset=%"PRId64,
	c->known, c->my_ply.found, c->their_ply.found, c->metadata.read_offset);

      enum meshms_status tmp_status;
      if (meshms_failed(tmp_status = open_ply(&c->my_ply, &iter->_my_reader)))
	return tmp_status;
      if (meshms_failed(tmp_status = open_ply(&c->their_ply, &iter->_their_reader)))
	return tmp_status;

      if (meshms_failed(tmp_status = update_their_stats(&c->metadata, &c->their_ply, &iter->_their_reader))){
	status = tmp_status;
	goto fail;
      }
      if (tmp_status == MESHMS_STATUS_UPDATED)
	status = tmp_status;
      ...
      if (meshms_failed(tmp_status = update_my_stats(&c->metadata, &c->my_ply, &iter->_my_reader))){
	status = tmp_status;
	goto fail;
      }
      if (tmp_status == MESHMS_STATUS_UPDATED)
...
enum meshms_status meshms_send_message(const sid_t *sender, const sid_t *recipient, uint8_t mark_known, const char *message, size_t message_len)
{
  ...
  if (message_ply_append(id, RHIZOME_SERVICE_MESHMS2, recipient, &conv->my_ply, b)==0){
    ...
    // save known conversations if something changed
    if (status == MESHMS_STATUS_UPDATED)
      write_known_conversations(m, conv);
